...
export default function PayEvent({ event, slotId, totalAmount, persons }: PayEventPageProps) {
    ...
    const handleRazorpay = async () => {
        ...
        try {
            ...
            const orderRes = await fetch(`/api/events/${event.id}/razorpay-order`, {
                ...
                body: JSON.stringify({ slot_id: slotId, amount: totalAmount, persons }), // send persons for webhook
            });
            ...
            const options = {
                ...
                handler: async function (response: any) {
                    // (Keep the handler for instant feedback, but webhook is now source of truth)
    ...
    const handleGooglePay = () => {
        // Show UPI QR or intent (for demo, just show a message)
                    ...
                    <CardContent>
                        {paymentStatus === 'success' ? (
                            <div className="p-6 text-center font-semibold text-green-600">
                                ...
                                <br />
                                <Button className="mt-4" onClick={() => router.visit('/events/browse')}>
                                    Back to Events
                                </Button>
                            </div>
                        ) : (
