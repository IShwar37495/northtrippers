...
export default function EditEvent({
    ...
    const [form, setForm] = useState({
        ...
        meal_included: eventData.meal_included,
        meal_times: eventData.meal_times,
        meal_price: eventData.meal_price || '',
        hotel_included: eventData.hotel_included,
        hotel_price: eventData.hotel_price || '',
        min_age: eventData.min_age,
        max_age: eventData.max_age || '',
        available_slots: eventData.available_slots,
        vehicle_id: eventData.vehicle_id,
        activities: eventData.activities,
        boarding_point: eventData.boarding_point,
        pickup_location_id: eventData.pickup_location_id,
        base_price: eventData.base_price,
    });

    const [loading, setLoading] = useState(false);
    const [photos, setPhotos] = useState<PhotoFile[]>([]);
    const [photoUrls, setPhotoUrls] = useState<string[]>(eventData.photos);
    ...
    useEffect(() => {
        ...
        for (let i = 1; i <= form.days; i++) {
            if (form.itinerary[i - 1]) {
                newItinerary.push(form.itinerary[i - 1]);
            } else {
    ...
    const addPhotos = (files: File[]) => {
        if (files.length + photos.length + photoUrls.length > 5) {
            toast({
                title: 'Error',
    ...
    const removePhoto = (index: number) => {
        if (index < photoUrls.length) {
            ...
            setPhotos((prev) => {
                // Get the photo to be removed before filtering
                const photoToRemove = prev[photoIndex];
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            router.put(`/events/${eventData.id}`, formData, {
                onSuccess: () => {
    ...
    return (
        <div className="min-h-screen bg-background">
            <Head title={`Edit ${eventData.name} - North Trippers`} />
                                        ...
                                        {allVehicles.map((vehicle) => (
                                            <SelectItem key={vehicle.id} value={vehicle.id.toString()}>
                                                <div className="flex items-center gap-3">
                                                    {vehicle.image_url ? (
                                                        <img src={vehicle.image_url} alt={vehicle.name} className="h-6 w-6 rounded object-cover" />
                                                    ) : (
                            ...
                            >
                                ...
                                {photoUrls.map((photo, index) => (
                                    <div key={`existing-${index}`} className="group relative">
                                        <div className="flex aspect-video items-center justify-center overflow-hidden rounded-lg border-2 border-dashed border-muted-foreground/25">
                                            <img src={photo} alt={`Event photo ${index + 1}`} className="h-full w-full object-cover" />
                                        </div>
                                        <Button
                                            type="button"
                                            variant="outline"
                                ...
                                {photos.map((photo, index) => (
                                    <div key={`new-${index}`} className="group relative">
                                        <div className="flex aspect-video items-center justify-center overflow-hidden rounded-lg border-2 border-dashed border-muted-foreground/25">
                                            <img src={photo.preview} alt={`New event photo ${index + 1}`} className="h-full w-full object-cover" />
                                        </div>
                                        <Button
                                            type="button"
                                            variant="outline"
                                            size="sm"
                                            onClick={() => removePhoto(photoUrls.length + index)}
                                            className="absolute top-2 right-2 border-red-500 bg-red-500 text-white opacity-0 transition-opacity group-hover:opacity-100 hover:border-red-600 hover:bg-red-600"
                                        >
                                            <X className="h-4 w-4" />
                                        </Button>
                                ...
                                {/* Upload Button */}
                                {photoUrls.length + photos.length < 5 && (
